//! Ceremony profile constants, and the protocol parameters governance owns.
//!
//! A Platform Profile fixes what a Platform Verifier pins: the tags it compares
//! an attestation against, the authority that must have answered, the method
//! and path it expects, how many attestations it verifies, and how the
//! Authorization Digest reaches it. This module is the single place those live,
//! so the notary that writes them and the verifier that compares them read one
//! definition.
//!
...
//! ceremony-common section 7. Every other libID-namespaced string --
//! `formatTag` (REQ-COMMON-53), `operationTag` and the platform name
//! (REQ-COMMON-55), and each Consumer's operation domain (REQ-COMMON-01A) --
//! is required to exist and required to be pinned, but its bytes are left to
//! the profile author.
//!
